...
{
    float * float_image=new float[64*64];
    convert(image, float_image);
    
    testNan(float_image, 64*64);
...
int main(int argc, char ** argv){
    ...
    */
    ...
    vector<string> imageTrainAdr=readImageAddress("trainingset.in");
    vector<string> imageTestAdr=readImageAddress("testingset.in");
    ...
    ////// SETTING FOR DeSTIN //////
    SupportedImageWidths siw = W64;
    uint centroid_counts[]  = {64,64,32,16,10};
    bool isUniform = true;
    ...
    cout << "[MESSAGE] DeSTIN network initialization completed" << endl;
    
    // Feature Exactor
    BeliefExporter * featureExactor=new BeliefExporter(*network, 2);
    ...
    int noIteration=1;
    int noLetters=1;
    int noTrainingImages=imageTrainAdr.size();
    int noTestingImage=imageTestAdr.size();
    int counter=0;
    
    cout << "[MESSAGE] TRAINING STARTED" << endl;
    for (int i=0;i<noIteration;i++)
    {
        for (int j=0;j<noTrainingImages;j++)
        {
            network->clearBeliefs();
            
            for (int k=1;k<=2*nLayers;k++)
            {
                counter++;
                float * float_image=callImage(trainingSIFTImages[j]);
                        
                network->doDestin(float_image);

                //cout << "[MESSAGE][TRAINING] Frame:" << counter << "\r";
                //cout.flush();
                /*
            ...
            cout << "[MESSAGE][TRAINING] IMAGE:" << j+1 << "\r";
            cout.flush();
        }
    }
    
    cout << "[MESSAGE] TRAINING COMPLETED" << endl;
    ...
    */
    ...
    for (int i=0;i<noTrainingImages;i++)
    {
        network->clearBeliefs();
        for (int j=1;j<=2*nLayers;j++)
        {
            counter++;
            float * float_image=callImage(trainingSIFTImages[i]);
            network->doDestin(float_image);
            
            //cout << "[MESSAGE][EXTRACT FEATURE] Output Frame:" << counter << "\r";
            //cout.flush();
        }
        
        featureExactor->writeBeliefToMat();
    ...
    for (int i=0;i<noTestingImage;i++)
    {
        network->clearBeliefs();
        for (int j=1;j<=2*nLayers;j++)
        {
            counter++;
            float * float_image=callImage(testingSIFTImages[i]);
            network->doDestin(float_image);
